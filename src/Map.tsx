...
const getBoatMarkerIcon = () => {
    return L.icon({
        iconUrl: getBoatMarkerSVG(),
        iconSize: [40, 40],
        iconAnchor: [20, 20],
        popupAnchor: [0, 0],
    })
...
const InnerMap = ({
    ...
    const [hasWsConnection, setHasWsConnection] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        setHasWsConnection((curHasWsConnection) => {
            if (curHasWsConnection) {
                return true;
            }
    ...
    return <Fragment>
        ...
        {Array.from(boatPositions.entries()).map(([uniqueKey, position]) => {
            ...
            return <Marker key={uniqueKey} position={[lat, lng]} icon={getBoatMarkerIcon()}>
                <Popup>
                    <div>{shipName}</div>
                </Popup>
            </Marker>
